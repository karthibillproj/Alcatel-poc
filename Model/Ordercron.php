 ...
 * Poc Config model
 */
class Ordercron extends \Magento\Framework\DataObject
{
    ...
    public function execute(){
        $this->mgrlog->info(time());
        $cancellabletime = $this->getcancellabletime(); 
        $cancellablestatus = $this->getcancellablestatus(); 
        $noncancellablestatus = $this->getnoncancellablestatus(); 
        $toDate = date('Y-m-d H:i:s', time());
      //  $from = '-'.$cancellabletime.' minutes';
        $from = '-5 day';
         $fromDate = date('Y-m-d H:i:s', strtotime($from)); 
        $orders = $this->_orderCollectionFactory->create()->addFieldToSelect('*')->addFieldToFilter('status', $cancellablestatus)
        ->addFieldToFilter('created_at', array('from'=>$fromDate, 'to'=>$toDate)); 
        $orderdetails = array();
         foreach($orders as $orderdata){
             $orderdata->setState(\Magento\Sales\Model\Order::STATE_PROCESSING, true);
             $orderdata->setStatus($noncancellablestatus);
             $orderdata->save();
         } 
    }
    ...
    public function orderupdate(){
         $order = $this->order;
         $cancellablestatus = $this->getcancellablestatus(); 
         $order->load('000000012');
          $this->mgrlog->info('test');
          $order->setState(\Magento\Sales\Model\Order::STATE_PROCESSING, true);
         //$order->setStatus($cancellablestatus);
          $order->setStatus('payment_accepted_cancellable');
